...
def collate_fn(batch, pad_val=-1):
    '''
    딥러닝 모델에서 패딩된 값은 실제 데이터가 아니므로, 모델이 학습하는 데 영향을 주지 않도록 주의해야 한다. \
    일반적으로 모델에 입력되는 데이터에 패딩을 적용할 때 마스크(mask)를 사용하여 패딩된 부분을 무시하도록 한다.
    '''
    '''
    예를 들어, 각각의 샘플들이 다른 길이의 시퀀스로 구성되어 있을 때, \
        collate_fn은 모든 시퀀스를 동일한 길이로 패딩하여 미니배치를 생성
    '''
    q_seqs = [] # 질문 시퀀스
    r_seqs = [] # 답변 시퀀스
    qshft_seqs = [] # 시프트 된 질문 시퀀스 : DKT 고려
    rshft_seqs = [] # 시프트 된 답변 시퀀스 : DKT 고려
    ...
    for q_seq, r_seq in batch:
        ...
        '''
        슬라이싱은 리스트나 배열에서 원하는 부분을 잘라내어 새로운 리스트나 배열을 만드는데 사용. 
        일반적인 형식은 start:end:step. 
        여기서 start는 시작 인덱스, end는 끝 인덱스(끝 인덱스는 포함되지 않음), step은 슬라이스의 간격. 
        '''
        ...
        # 각 원소가 -1이 아니면 True, -1이면 False로 값을 채움
...
def recorder(test_score, record_time, config):
    ...
    #  .extend() 메서드는 리스트에 여러 개의 요소를 추가하는 데에 유용. 
    #  만약 단일 요소를 추가하려면 .append() 메서드를 사용.

    append_list.append(config.crit + "_test_score")
    append_list.append(test_score)

    # csv 파일 열어서 한 줄 추가해주기
