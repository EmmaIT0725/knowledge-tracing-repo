...
# ALGEBRA2005는 algebra2005_loader 모듈에서 정의된 클래스(Class)
...
# ASSIST2015는 assist2015_loader 모듈에서 정의된 클래스(Class)
...
def get_loaders(config, idx=None):
    ...
    # 1. dataset 선택
    if config.dataset_name == "algebra2005":
        dataset = ALGEBRA2005(config.max_seq_len)
        num_q = dataset.num_q       # "개념"(문제의 컨셉정보)의 개수가 몇 개인지 
                                    # >> 어떤 데이터는 개념 정보만 필요할 수도 있고, 문제 번호 정보도 같이 필요할 수 도 있음(_pid 처럼)
        num_r = dataset.num_r       # "학생 응답"이 몇 개인지
        num_pid = None              # "질문 id"(문제의 번호)가 몇 개인지
        num_diff = None             #  "난이도"가 몇 개인지
        collate = collate_fn        
        # collate : 데이터 로더에서 최종적으로 나오는 데이터 포맷을 맞춰주는 것
        # collate_function은 정의를 해주어야 함.
        ...
        # dkt 모델은 collate 구현방식이 다르므로 따로 function을 만들어 놓음
        ...
        num_diff = None
        collate = pid_collate_fn
    # elif config.dataset_name == "assist2015":
    ...
    ### 2. 이 부분 다시 코딩하기 >> 없앨 수 있는 방법?! >> 굳이 이렇게 한 이유?
    ...
    # first_chunk: 데이터셋의 처음 20% 부분
    # second_chunk: 데이터셋의 20%에서 40% 사이의 부분
    # third_chunk: 데이터셋의 40%에서 60% 사이의 부분
    # fourth_chunk: 데이터셋의 60%에서 80% 사이의 부분
    # fifth_chunk: 데이터셋의 나머지 (80%에서 끝까지) 부분

    # 각 부분은 PyTorch의 Subset 클래스로 생성되며, 
    # 이 클래스는 주어진 데이터셋에서 특정 인덱스 범위에 해당하는 부분 데이터셋을 나타낸다. 
    # 각 부분은 원본 데이터셋에서 인덱스에 따라 선택된 샘플들을 포함.
    ...
    valid_loader = DataLoader(
        ...
        shuffle = False,
        collate_fn = collate
        # valid_loader는 학습을 하는 것이 아니기 때문에 shuffle = False
    )
    test_loader = DataLoader(
        ...
        shuffle = False, 
        collate_fn = collate
        # test_loader는 학습을 하는 것이 아니기 때문에 shuffle = False
    )
